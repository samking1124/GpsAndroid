...
public class MonitorClient extends Thread {
		...
		try {
			...
			if ("{POS".equals(cmd[0])) {
				GeoPoint point = new GeoPoint(
						(int) (Double.parseDouble(cmd[2]) * 1E6), (int) (Double
								.parseDouble(cmd[3]) * 1E6));
	...
	public void updateTracker(String addTrackerNos, String delTrackerNos) {
		...
		if (addTrackerNos != null && !addTrackerNos.trim().equals("")) {
			ArrayList<Marker> latestMarkers = HttpClientUtil
			...
			if (latestMarkers != null && latestMarkers.size() > 0) {
				...
				// 调整地图中心和缩放级别以能看到所有跟踪器
				// for(Marker marker:latestMarkers){
				// monitorActivity.getMapController().zoomToSpan(marker.getPoint().getLatitudeE6(),
				// marker.getPoint().getLatitudeE6());
				// }
				// 添加所有被监控跟踪器的最新位置
				monitorActivity.getMarkerOverlay().addMarkers(latestMarkers);
			}
			// 发送添加监控跟踪器命令到服务器
